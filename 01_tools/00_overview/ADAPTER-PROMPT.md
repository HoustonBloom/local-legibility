...
ALL outputs you produce go inside the **packet folder** (the folder this prompt lives in), specifically into the packet's `03_outbox/` subfolder. The packet ships with `03_outbox/` already created at the packet root.

- First Look audit → `<packet-root>/03_outbox/first-look.html`
- Generated frames → `<packet-root>/03_outbox/frames/YYYYMMDD_<slug>.html`
- Cut configs → `<packet-root>/03_outbox/frames/YYYYMMDD_<slug>_config.md`
- Bundled windows → `<packet-root>/03_outbox/<window-name>.html`
...
Do NOT write outputs to the user's content folder, to the project root above the packet, or to your current working directory. Always inside `<packet-root>/03_outbox/`.
...
6. Save to `<packet-root>/03_outbox/frames/YYYYMMDD_<project-slug>-pitch.html` (the `-pitch` suffix is mandatory · matches the lens-in-filename rule).
...
>
> Your pitch is at: `<packet-root>/03_outbox/frames/<filename>.html`
...
3. **The First Look signpost with the EXACT absolute path** (literal path, no placeholder syntax, so the user can click or navigate to it directly):
   > "Not sure? Open <packet-root>/03_outbox/first-look.html in your browser : it shows what I found, ranks every lens against your content, and lets you preview each lens before you pick one. Every lens has a Copy prompt button."
...
> Your frame is at: `<packet-root>/03_outbox/frames/<filename>.html`
...
The AI will drop a **First Look audit** at `<packet-root>/03_outbox/first-look.html`. Open that file in your browser first : it shows what the AI found, ranks every lens against your content, and lets you preview what each lens produces before you pick one. Every lens has a Copy prompt button right on the page.
...
If the `<packet-root>/03_outbox/first-look.html` file isn't there, tell the AI "produce the first-look audit before anything else" : that's the starting artifact and it should exist before the conversational reply lands.
