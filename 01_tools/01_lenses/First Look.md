...
The kit's onboarding lens. Produced automatically by the adapter prompt the moment a new user runs it against their folder. Lands in their folder at `03_outbox/first-look.html`.
...
- **Output location:** the user's `03_outbox/` folder, as `first-look.html`. The AI must reference this file by its absolute path in its closing reply.
...
- **Prompt format (all lenses, consistent):** `Cut me a frame using the [lens-name] lens. [one-line qualifier]. Save to 03_outbox/frames/ and tell me the exact path.` Do not vary the wrapper : only the qualifier changes per lens.
...
- Saves to user's `03_outbox/first-look.html`.
